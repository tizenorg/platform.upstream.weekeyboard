...
_wkb_ui_verify(struct weekeyboard *wkb)
{
   ...
   const char *theme = wkb_ibus_config_get_value_string("general", "theme");
   if (strcmp(theme, wkb->theme) != 0)
     {
        DBG("_wkb_ui_verify - redrawing keyboard, theme = <%s>", theme);
        strncpy(wkb->theme, theme, PATH_MAX);
...
_wkb_ui_setup(struct weekeyboard *wkb)
{
   ...
   if (strlen(theme) == 0 || strcmp("default", theme) == 0)
     {
